...
quiz(caption = '',
     ...
     question("Which of the following questions are best addressed using Pearson's correlation? Please select all that apply.",
              ...
              answer('Can average sleep (hours per week) predict response time (in milliseconds)?'),
              random_answer_order = T,
              message = "Pearson's correlation is used to test for an association between two continuous variables. It doesn't test prediction or for group differences, and it should not be used with categorical data."))
```
...
In the left column, set the type of plot to 'Boxplot', the y-axis to `AntisocialBehaviour` and the x-axis to `SEcategory`. You can leave everything else as the default setting. To make sure the plot is APA style, navigate to the options on the right of the page, click Theme, and select 'classic' from the drop-down box. You should now have an APA-style boxplot that shows the difference in antisocial behaviour between self-enhancement value groups:
...
In the left column, set the type of plot to 'Scatter', the y-axis to `AntisocialBehaviour` and the x-axis to `SEcategory`. Tick the 'Show regression line' box, and make sure the smoothing function is 'lm'.  To make sure the plot is APA style, navigate to the options on the right of the page, click Theme, and select 'classic' from the drop-down box. You should now have an APA-style scatterplot that shows the association between self-enhancement values and antisocial behaviour:
...
<span style = "font-weight: bold; font-size: 14pt"> Boxplot </span>
In the left column, set the type of plot to 'Boxplot'. The y-axis can stay set at `AntisocialBehaviour`, but change the x-axis to `SEcategory`. You can leave everything else as the default setting. You should now have an APA-style boxplot that shows the difference in antisocial behaviour between self-enhancement value groups:
...
In the left column, set the type of plot to 'Dot + Error'. The y-axis can stay set at `AntisocialBehaviour` and the the x-axis at `SEcategory`, but add `ParentalPressure` as the Group variable. You can leave everything else as the default setting. 
...
In this study, we investigated whether adolescent athletes' self-enhancement values were associated with their antisocial behaviours towards members of the opposing team To do this, we collected data from a sample of 173 adolescents who participate in team sports. One participant was excluded due to an error in the data. Our final sample consisted of 172 adolescent athletes between the ages of 13-18 ($M$ = 15.92, $SD$ = 1.60; 88 Females, 84 Males). 

We ran a Pearson's correlation to test the association between scores on a self-enhancement values scale and the average score on a scale that measured antisocial behaviour. We also used a Pearson correlation to test whether scores on each of the four questions on the self-enhancement scale were associated. $\alpha$ was set at .05 for all analyses.

Participants scored an average of 12.38 on the self-enhancement scale ( $SD$ = 2.75; minimum = 6.28; maximum = 20.79) and had an average antisocial behaviour rating of 2.99 ( $SD$ = 0.87, minimum = 1, maximum = 5). Results from a Pearson's correlation indicated that there was a moderate, positive relationship between the two variables, r(170) = .31, p < .001, 95% CI = [.17, .44], indicating that participants who endorsed stronger self-enhancement values also demonstrated more antisocial behaviours towards members of the opposing team (see Figure 1). 
...
Additionally, scores on each of the 4 questions on the self-enhancement scale were moderately positively correlated (see Table 1).


```{r, echo = F, fig.align='center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/week7_week7_scatterFig.pdf')
```
