...
7. Produce an APA-style correlation matrix
...
You can download the dataset from this experiment [here](https://mtruelovehill.github.io/PRM/Labs/RegressionLabData.sav). It contains the following variables:

```{r, echo = F}
dat <- read.csv('https://mtruelovehill.github.io/PRM/Labs/RegressionLabData.csv')
...
$H_1:$ Sleep predicts well-being in university students when controlling for the effects of physical activity and social support. Specifically, more sleep will be associated with a greater sense of well-being.
...
quiz(caption = 'Check Your Results',
     question_numeric("What is the R2 value associated with your sensitivity power analysis (i.e., what R2 value are you capable of detecting with 600 participants)? Please round your answer to 2 decimal places.",
              answer(.02, correct=T),
              message = "You are capable of detecting an R2 value of .02. The power analysis produced an f2 value of .02. You'll need to convert f2 to R2. In this example, it's basically identical, but this is not always the case."))
```
...
```{r, eval = F, echo = F}
     question_numeric("How many participants do you need to detect an R2 of .13 with 3 predictors?",
                      answer(77, correct = T),
                      message = 'First, you need to convert R2 to f2. Using f2, the power calculation indicates that 77 participants are needed to detect this effect with 80% power and alpha = .05.')
```
...
When performing regression, it is a good idea to check and report the associations between your predictors and the outcome. As you are looking for associations between continuous variables, you'll use a Pearson correlation in cases where variables are normally distributed, and Spearman's $\rho$ in cases where they are not. 

Recall that $r$ (and $\rho$) has the following interpretation standards:
...
+ [ ] $\ $ Run correlations and interpret the relationships between the variables that will be included in your regression model. 

</div>

```{r cyrCorr, echo = FALSE}
quiz(caption = 'Check Your Results',
     question_numeric("What is the Pearson correlation value for the association between PhysicalActivity and Sleep? Please round your answer to two decimal places.",
              answer(.20, correct = T),
              message = "Pearson's r for the association between PhysicalActivity and Sleep is .20."), 
     question_numeric("What is the Pearson correlation value for the association between SocialSupport and Wellbeing? Please round your answer to two decimal places.",
              answer(.16, correct = T),
              message = "Pearson's r for the association between SocialSupport and Wellbeing is .16."))
```
...
Navigate to *Analyze > Correlate > Bivariate* and add each of your continuous variables (`Wellbeing`, `SocialSupport`, `Sleep`, and `PhysicalActivity`) to the 'Variables' box. Click 'Show only the lower diagonal'; this removes redundant information from the correlation matrix. Click 'Continue' and then 'OK'. 

The output provides a correlation matrix with the value of $r$ and the p-value. Consider the correlations alongside the predefined thresholds, above.

<b>Negligible associations:</b>

+ `SocialSupport` and `PhysicalActivity`

<br>

<b>Weak associations:</b>

+ `PhysicalActivity` and `Wellbeing`
+ `SocialSupport` and `Wellbeing`
+ `SocialSupport` and `Sleep`

<br>

<b>Moderate assocations:</b>

+ `PhysicalActivity` and `Sleep`

<br>

<b>Strong associations:</b>
