...
Today, you'll be using interactive visualisations to demonstrate the concepts covered in the lecture.
...
Recall from the lecture that the **standard deviation** reflects the general deviation from the mean within a single sample, while the **standard error** is the general deviation from the mean of the means between samples.
...
Recall that standard error can be estimated from a single sample with the following equation:

$$SE = \frac{\sigma}{\sqrt{n}}$$

```{r tyuSE, echo = FALSE}
quiz(caption = 'Test Your Understanding',
     question('Refer to the equation for standard error above. What is the relationship between sample size and standard error?',
...
You'll also note that while each sample's mean falls near the true population value, it is uncommon for a sample mean to perfectly capture the population mean. Confidence intervals are useful because they are more likely than the sample mean to capture the population mean.
...
## Power Analyses
We'll begin to conduct power analyses when we run specific statistical tests. Today, focus on improving your understanding of the concept of statistical power. Have a look at the following power curves and spend some time trying to interpret them.
