...
Your interaction plot showed a small increase in the association between `Physical Association` and `Wellbeing` in participants with higher levels of sleep as compared to those with moderate or lower than average levels of sleep. However, it would be good to have statistical results to back this visualisation up. To get these, you need to perform a simple slopes analysis.
...
+ [ ] $\ $ Mean-centre your moderating variable, `Sleep`
...
<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt"> Solution - Mean-centre `Sleep` </span></summary>
...
To do this in SPSS, navigate once again to *Transform > Compute Variable*. Add your a name for your new high sleep variable. I used `HiSleep`. Subtract the $SD$ of `Sleep` from your new mean-centered sleep variable, `MCSleep`:
...
You'll do this in the same way you created a basic interaction term, just with slightly different variables. First, create an interaction term that tests the interaction in participants who sleep more than average. Once more, navigate to *Transform > Compute Variable*. Name your new variable `HiInt` and set it as equal to the product of `HiSleep` and `PhysicalActivity`.
...
Finally, repeat the process one last time, but with `MCSleep`:

```{r, echo = F, fig.align= 'center', out.width = '75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/week9_modIntVar.png')
```

If you've created all of these new variables properly, you'll now have 6 new variables to run 3 new interactions:
...
Now, you'll run 3 separate interaction models to capture the association between `PhysicalActivity` and `Wellbeing` at each level of `Sleep`. First, let's look at the association in participants with low levels of sleep.

<span style = "font-weight: bold; font-size: 14pt"> Participants with Lower than Average Sleep </span>

Navigate to *Analyze > Regression > Linear*. Click the 'Reset' button at the bottom to clear out any previous information. Move `Wellbeing` to the 'Dependent' box. In the lower box, add `PhysicalActivity`, `SocialSupport`, `LoSleep` and `loInt`, then click 'OK'. You can ignore the ANOVA box, as the overall model effects will remain the same. The only thing that will change is the the effect on `PhysicalActivity`. Here, you can see the $p$-value = .526. This indicates that there is no association between physical activity and well-being in students who get lower than average amounts of sleep.
...
Navigate to *Analyze > Regression > Linear*. Click the 'Reset' button at the bottom to clear out any previous information. Move `Wellbeing` to the 'Dependent' box. In the lower box, add `PhysicalActivity`, `SocialSupport`, `MCSleep` and `modInt`, then click 'OK'. In the coefficients box, the $p$-value for `PhysicalActivity` is .253 This indicates that there is no association between physical activity and well-being in students who get average amounts of sleep.
