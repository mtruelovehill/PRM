...
Click 'Continue', then 'OK'. The variable `Session` will appear in your Data View window. 

To change the labels of this variable, navigate to  **Variable View** and click the corresponding *Values* cell. Add a value of 0 and set the Label to 'AM'. Add a label of 'PM' to the value of 1. Click 'OK'. You might also want to change the decimal places under the *Decimals* column to 0.
...
### Your Task 
+ [ ] $\ $ Use each of the score variables (`A`, `B`, and `C`) to calculate 3 new variables that reflects the raw score as a percentage. 
+ [ ] $\ $ Name the new variables `Apercent`, `Bpercent`, and `Cpercent`
...
Make sure to use quotes around 'Student'. This is case-specific, so make sure you type 'Student' and not 'student'. Click 'Continue', then 'OK'. A new filtering variable (`filter_$`) will appear, and slashes will appear over the row numbers of the 'RA' variables. As long as the filtering variable exists, they will not be included in any of the steps you take going forward.

As the remaining lab tasks for today require you to use this filtered dataset, you should leave the filter in place. However, if you'd ever like to remove a filter, simply delete the filter variable. 
...
### Your Task 
+ [ ] $\ $ Check the frequency of `Gender` and `Session`. Make sure to display frequency tables.
...
Notice that if you've set labels properly in previous steps, you should see the names (e.g., 'AM'/'PM') listed in the frequency tables, rather than numeric values. If you have different values in your frequency tables, check that you've properly filtered the data.
...
### Your Task 
+ [ ] $\ $ Produce plots to visualise the distribution of the variables `Gender`, `Session`, `Age`, `Apercent`, `Bpercent`, and `Cpercent`
...
Select *Analyze>Descriptive Statistics>Frequencies*. Add your categorical variables to the 'Variable(s)' box, uncheck 'Display frequency tables', and then click 'Charts...'. Select 'Bar charts'. Note at the bottom that you can either plot the raw frequencies or percentages. Here, you can leave 'Frequencies' checked. Click 'Continue', then 'OK'.  
...
Select *Analyze>Descriptive Statistics>Frequencies*. Add your categorical variables to the 'Variable(s)' box, uncheck 'Display frequency tables', and then click 'Charts...'. Select 'Histograms', then tick 'Show normal curve on histogram.' Click 'Continue', then 'OK'. 

Have a look at the output. If a variable is normally distributed, most observations will fall near the mean, while more extreme values are less common. The bars should generally fall along the normal curve.
...
### Your Task 
+ [ ] $\ $ Check the measures of central tendency discussed in class for `Gender`, `Session`, `Age`, `Apercent`, `Bpercent`, and `Cpercent`. Make sure to use the proper central tendency measures given the variable type.
+ [ ] $\ $ For all of the above continuous variables, check the measures of variance described in class. These include the maximum, minimum, range, interquartile range, and standard deviation.

<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt">Click here for a hint </span></summary>

You'll be using *Analyze>Descriptive Statistics>Frequencies* again.
...
Note that `Gender` and `Session`, as categorical variables, should only be described using Mode. The other variables can be described using all 3 measures.
...
*Note.* This table reflects the percentages, rather than the raw scores on each assessment.
...
Assessment C had a reasonable average, as well as higher variability in the range of scores (see Figure 2). This indicates that, of these assessments, Assessment C may best capture the range of student abilities within the course.
...
quiz(caption = '',
     ...
     question_numeric("How many total datapoints do you have for the gender variable?",
              answer(45, correct=T),
              message = "If you've properly filtered the data, you should have 45 total datapoints for the gender variable."),
     question_numeric("What percentage of participants are female?",
              answer(42.2, correct=T),
              tolerance = .1,
              message = "If you've properly filtered the data, 42.2% of the students are female."),
     ...
     question_numeric("What is the average percentage on Assessment A?",
              answer(39.09, correct=T),
              tolerance = .01,
              message = 'The mean score on Assessment A is 39.09.'),
     question_numeric("What is the most common age in this sample?",
              answer(19, correct=T),
              message = 'The most common age in this sample is 19'),
     question_numeric("What is the standard deviation of percentages on Assessment C? Please report your answer to two decimal places.",
              answer(9.44, correct=T),
              tolerance = .01,
              message = 'The standard deviation of percentage scores on Assessment C is 9.44'),
     question_numeric("What is the IQR for percentages on Assessment B?",
              answer(4, correct=T),
              message = "To get the IQR for a variable, you'll need to subtract the 25th percentile (94) from the 75th percentile (98). This gives an IQR of 4.")
     )
```

## Usefulness of Central Tendancy
