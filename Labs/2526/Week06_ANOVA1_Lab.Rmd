...
4. Run a power analysis for a one-way ANOVA
5. Interpret and report results from one-way ANOVAs

If you also complete the bonus material, you'll be able to:

1. Run an appropriate power analysis for a repeated-measures ANOVAs
2. Check assumptions for a repeated-measures ANOVA 
3. Use SPSS to perform a repeated-measures ANOVAs
4. Interpret and report results from repeated-measures ANOVAs


## Overview

You'll be working with data adapted from studies on emotion recognition (see [here](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0243708)). Specifically, you're testing whether the region of the face covered affects a child's ability to recognize emotions conveyed by facial expressions.

To investigate this, you recruited a sample of primary-school aged children. All participants were presented with photos of people making facial expressions associated with 5 different emotions (happiness, sadness, fear, anger, disgust). 

However, participants were grouped into one of three conditions. Participants in the first condition viewed unobstructed faces Participants in the second condition viewed faces wearing sunglasses so that the eyes were obstructed. Participants in the third condition viewed faces wearing masks so that the mouth was obstructed (see examples below).

```{r, echo = F, out.width='90%', fig.align='center'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_expStimuli.png')
```
...
You can download the dataset from the first experiment [here](https://mtruelovehill.github.io/PRM/Labs/ANOVA1lab.sav). It contains the following variables:

```{r, echo = F}
dat <- read.csv('https://mtruelovehill.github.io/PRM/Labs/ANOVA1lab.csv')
...
quiz(caption = 'Check Your Results',
     question_numeric("How many participants do you need for your One-Way ANOVA to have 80% power to detect a moderate effect with alpha = .05?",
              answer("158", correct = T),
              message = "You will need 158 total participants for this analysis to have an 80% chance of avoiding a Type II error."),
     question('What effect size does WebPower use in power calculations for One-Way ANOVAs?',
              answer('f', correct = T),
              answer("Cohen's d"),
              answer('eta squared'),
              answer('r'),
              random_answer_order = T,
              message = "While SPSS (and this course) will use eta squared as our effect size for ANOVA, WebPower uses f. You'll have to convert eta-squared to f to get correct results for your power analysis."))
```
...
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_powerAnalysisChoice.png')
```
<br>
...
```{r, echo = F, fig.align='center', out.width='50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_oneWayPower.png')
```
...
+ [ ] $\ $ Add a key to the `Group` variable so that 0 = 'Control', 1 = 'Mouth Obstructed' and 2 = 'Eyes Obstructed'
...
In this instance, SPSS assigned the proper measures by default (with the exception of ID, which won't be used as part of any analysis, so you ignore it). No changes are required.
...
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_GenderLabels.png')
```

Click 'OK'. If you've done this properly, you'll see that each numeric value is now associated with a character label. Repeat this process for the `Group` variable, setting 0 = 'Control', 1 = 'Mouth Obstructed', and 2 = 'Eyes Obstructed'.
...
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_freqTables.png')
```

Note that if you set up the labels for the `Group` and `Gender` properly, these will appear in the frequency tables, rather than the numeric values you see in 'Data View'. 
...
```{r, echo = F, fig.align= 'center', out.width = '75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_descStats.png')
```
...
```{r, echo = F, fig.align= 'center', out.width = '75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_corrHist.png')
```
...
Have a look at the [lecture slides](https://mtruelovehill.github.io/PRM/Lectures/Week05_ANOVAI_lecture.html#1) if you're lost. Because normality must be tested within levels, you should use *Analyze>Descriptive Statistics>Explore* to get the plots you need. To get the plot for homogeneity of variance, you need to save the residuals and predicted values of the ANOVA model using *Analyze>General Linear Model>Univariate*. 
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_histogramControl.png')
```

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_histogramEyesObs.png')
```

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_histogramMouthObs.png')
```
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_qqControl.png')
```

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_qqEyesObs.png')
```

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_qqMouthObs.png')
```
...
Navigate to *Analyze>General Linear Model>Univariate*. Add the independent variable column, `Group`, to the 'Fixed Factor(s)' box, and add `Correct` to the 'Dependent Variable' box. Click 'Save', and tick the 'Unstandardized' option under 'Predicted Values' and the 'Standardized' option under 'Residuals'. Click 'Continue', then 'OK'. Some tables will appear in the Output window, but you need to instead look at the Data window. Notice you have two new columns, `PRE_1`and `ZRE_1`. You will need to plot these two variables to assess heterogeneity. 

Click 'Graphs', select 'Scatter/Dot', and then 'Simple Scatter.' Place the `Predicted Value` variable in the 'X-axis' box and the `Standardized Residual` variable in the 'Y-axis' box, then click OK. Have a look at the plot that is produced. Here, there isn't much difference in the spread between each column of points, so we can say that the assumption of homogeneity is met. 

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_hetPlot.png')
```
...
quiz(caption = 'Check Your Results',
     question_numeric("What is the F-statistic associated with your analysis? Please round your answer to two decimal places.",
              answer(15.14, correct = T),
              message = 'The F-statistic associated with this test is 15.14.'))
```

<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt"> Click here for a hint </span></summary>

To run a One-Way ANOVA, you should navigate to *Analyze>General Linear Model>Univariate*
...
Navigate to *Analyze>General Linear Model>Univariate*. Add the independent variable column, `Group`, to the 'Fixed Factor(s)' box, and add `Correct` to the 'Dependent Variable' box. To perform post-hoc tests, click 'Post-Hoc', and move `Group` to the 'Post Hoc Tests for:' box. Select 'Tukey', then click 'Continue.' If you'd like to produce a plot showing the differences between each level, you can also tick 'Plots', then put `Group` into the 'Horizontal Axis' box, then click 'Add'. Select 'Bar Chart' and tick 'Include Error Bars', then click 'Continue'. Finally, click 'Options', and select 'Descriptive Statistics', and 'Estimates of Effect Size', then click 'Continue', then 'OK'. 

You'll see the following output for your One-Way ANOVA:

```{r, echo = F, fig.align= 'center', out.width = '75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_results.png')
```

The overall model results are shown in the first row ('Corrected Model'). If you look at the .sig column, you can see that the $p$-value is less than .001, which is smaller than our $\alpha$ threshold of .05. This means that our results are significant. Specifically, there is a less than 0.1% chance that we would have differences this large in recognition between facial obstruction groups if the null hypothesis were true. This tells us that there is a significant difference in the case of least one of our pairings. 
To get a sense of how meaningful this difference is, we check the effect size. $\eta^2$ is equal to .146. 
...
These tables provided the results and effect size from the overall group comparison, but it's important to understand which groups are actually different from each other. To determine this, post-hoc analysis should be conducted. If you checked the Tukey's posthoc option when setting up your analysis, you should see the following output:

```{r, echo = F, fig.align= 'center', out.width = '75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_posthocs.png')
```

If you don't see the labels and instead see numbers in the first two columns, it just means you didn't actually add labels to your `Group` variable in *Variable View*. The results should still look the same.
...
Note that the numbers inside the curly brackets refer to the degrees of freedom. ANOVA has two types of $df$ that should be reported. Please refer to the [lecture slides](https://mtruelovehill.github.io/PRM/Lectures/Week05_ANOVAI_lecture.html#1) for a specific example.
...
### Your Task
<div class="nobullet">
+ [ ] $\ $ Run a power analysis using $\alpha$ = .05 and power = .8 to determine the sample size necessary to detect a moderate effect size ( $\eta^2$ = .06) using a Repeated-Measures ANOVA. 
</div>

Note that WebPower uses $f$ as its measure of effect size for ANOVA, so you'll need to convert $\eta^2$ to $f$ before running the power analysis. You can do this with the following equation:
...
The value of $f$ will be the same as the one you computed in [Power Analysis]
...
```{r, echo = F, fig.align='center', out.width = '50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_RMpowerChoice.png')
```
<br>
...
```{r, echo = F, fig.align='center', out.width = '50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_RMpower.png')
```
...
You can download the dataset from the second experiment [here](https://mtruelovehill.github.io/PRM/Labs/anovaIBonusData.sav). It contains the following variables:

```{r, echo = F}
dat <- read.csv('https://mtruelovehill.github.io/PRM/Labs/anovaIBonusData.csv')
datInfo <- data.frame(VariableName = colnames(dat),
                      Description=c('Participant ID',
                                    'Levels: 1 = Female; 2 = Male',
                                    'Age in years',
...
+ [ ] $\ $ Task 4: Run a repeated-measures ANOVA to test your hypothesis; make sure to also check the effect size.
...
```{r cyrBonus, echo = F}
quiz(caption = 'Check Your Results',
     question_numeric("What was the F-statistic associated with emotion? Please round your answer to two decimal places.",
              answer("179.65", correct = T),
              message = "The F-statistic associated with emotion is 179.65."),
     question('Is the difference in recognition rate between happiness and anger significant?',
              answer('Yes', correct = T),
              answer('No'),
              message = 'The p-value associated with the comparison between happiness and anger is < .001, which is less than our alpha threshold of .05. We would consider this result significant.'),
     question('Is the difference in recognition rate between fear and disgust significant?',
              answer('Yes'),
              answer('No', correct = T),
              message = 'The p-value associated with the comparison between fear and disgust is .726, which is greater than our alpha threshold of .05. We would not consider this result significant.'))
```


<span style = "font-weight: bold; font-size: 16pt"> Task 1: State Your Research Question, Hypotheses, and Variables </span>
<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt"> Click here for a hint </span></summary>

Have a look at the [lecture slides](https://mtruelovehill.github.io/PRM/Lectures/Week05_ANOVAI_lecture.html#1) if you need guidance here.
...
<span style = "font-weight: bold; font-size: 16pt"> Task 2: Check Experiment 2 data </span>

<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt"> Click here for a hint </span></summary>

You can follow the steps from [Data Review]
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_RMdesc.png')
```
...
<span style = "font-weight: bold; font-size: 16pt"> Task 3: Check Experiment 2 assumptions </span>

<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt"> Click here for a hint </span></summary>

As usual, you can use histograms and Q-Q plots to assess the normality of each of your recognition variables. To assess sphericity, you can use Mauchly's test of sphericity. You can use this flowchart to help you make a decision about sphericity:

```{r, echo = F, fig.align = 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_sphericity.png')
```
...
```{r, echo = F, fig.align= 'center', out.width='50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_happinessHist.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_sadnessHist.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_fearHist.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_angerHist.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_disgustHist.png')
```
...
```{r, echo = F, fig.align= 'center', out.width='50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_happinessQQ.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_sadnessQQ.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_fearQQ.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_angerQQ.png')
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_disgustQQ.png')
```
...
```{r, echo = F, fig.align = 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_mauchley.png')
```
...
<span style = "font-weight: bold; font-size: 16pt"> Task 4: Run RM ANOVA & get effect sizes </span>
...
```{r, echo = F, fig.align= 'center', out.width='50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_RMfactors.png')
```

Add each of your 5 recognition variables to the 'Within-Subjects Variables' box. To check for differences between each emotion, you'll need to click 'EM means' and add 'Emotion' to the 'Display Means for' box. Tick the 'Compare main effects' box, then select 'Bonferroni' under the 'Confidence interval adjustment' dropdown. The Bonferroni adjustment is used to correct for multiple comparisons. If you've done everything properly, it should look like this: 

```{r, echo = F, fig.align= 'center', out.width='50%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_EMmeans.png')
```


Click 'Continue'. Next, click the 'Options' box. You've already calculated descriptive statistics (e.g., mean/sd) in an earlier step, so you can leave that blank. Click 'Estimates of effect size' to produce your effect size measures. Click 'Continue'. 
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_RMeffects.png')
```

Because sphericity was violated, you cannot interpret the results on the line that says, 'Sphericity Assumed.' Instead, you need to make a decision about whether to intepret the Greenhouse-Geisser or Huynh-Feldt line. These methods are corrections that adjusts data that lack sphericity so the results can be interpreted with confidence. You can use this flowchart:
...
As the Greenhouse-Geisser $\epsilon$ is equal to .953, which is greater than 0.75, you should interpret the results from the line that says, 'Huynh-Feldt'. 

In this table, you can see that the $F$-statistic of 179.65 is much larger than the null value of 1, so, unsurprisingly, the $p$-value (< .001) is below the $\alpha$ threshold of .05. We can consider the results to be significant. The effect size ($\eta^2_p$, AKA partial $\eta^2$) is equal to .47, which is equivalent to a strong effect (see the solution box from [Analysis] for interpretation standards). This tells us that the type of feeling being emoted does affect a child's ability to recognise it in masked individuals. 

To determine which emotions are significantly different from each other, scroll down to the 'Pairwise Comparisons' box. 

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_pwc.png')
```
...
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/anovaI_emotionKey.png')
```
...
<span style = "font-weight: bold; font-size: 16pt"> Task 5: Interpret & Report </span>

<div class="container">
<details><summary><span style = "font-weight: bold; font-size: 16pt"> Click here for a hint </span></summary>

See the hint from [Interpret & Report].
