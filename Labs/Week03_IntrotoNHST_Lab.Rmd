...
output$effectSizePlot <- renderPlot({
  ...
  ggplot(data.frame(x=c(15, 85)), aes(x)) +
    ...
    theme(axis.text.x = element_text(size = 12),
          ...
          axis.title.y = element_blank(),
          axis.ticks.y = element_blank()) +
    annotate(geom = 'text', label = paste('Effect Size =', effSize), x = 25, y = .19, size = 6) +
  ...
  observeEvent(input$do, {
    CIs <- data.frame(Sample = 1:100, mean = rep(NA,100), lowBound = rep(NA, 100), hiBound = rep(NA, 100))
    ciDat <- data.frame(Cscore = rnorm(10000, mean = 58, sd = 7))
    ...
    output$ciPlot <- renderPlot({
      ggplot(CIs, aes(mean, y = Sample)) +
        ...
        scale_y_continuous(limits = c(1, 100), breaks = seq(0, 100, by = 1)) +
...
Recall that you had a small sample of students who completed each of the four assessments. The optimal assessment was determined to be Assessment C. Imagine that the instructor adopted Assessment C for use in their course. The plot below reflects the probability distribution of assessment performance across all students on the course. 
...
quiz(caption = 'Test Your Understanding',
     ...
     question_numeric('Assume this distribution is perfectly normal. What was the mean score on the assessment?',
                      answer(58, correct = T),
                      message = 'In a normal distribution, all observations are centred around then mean. The mean score on this assessment is 58, as 50% of students scored 58 or above, and 50% of students scored 58 or below.')
)
```
...
Before running a test, we use $\alpha$ to determine our cutoff for significance. $\alpha$ refers to the proportion of values in the null distribution we consider to be extreme. 
...
Recall from the [lecture](https://mtruelovehill.github.io/PRM/Lectures/Week03_NHST_lecture.html#37) that a $p$-value is the probability of obtaining a statistical test result at least as extreme as ours, given that the null hypothesis is true. Keep in mind that we're interested in the probability of values **at least as extreme as our value**, regardless of whether they are positive or negative.

The plot below shows the probability of values on either side of the line representing the test statistic value. Use this plot to answer the following questions.
...
quiz(caption = 'Test Your Understanding',
     ...
     question('Which of the following test statistics are in the most extreme 1% of the data?',
...
Use the plot below to answer the following questions. Remember that the shaded area reflects $p$ (the probability of getting a test statistic at least as extreme as yours given the null is true).
...
quiz(caption = 'Test Your Understanding',
     ...
     question("If alpha = .01, is the p-value associated with a test statistic of -2.55 significant?",
               answer('Yes', correct = T),
              answer('No'),
              message = "For a test statistic of -2.55, p = .010. As this is equal to the alpha threshold of .01, this result would be significant."),
     question_numeric('Which test statistic value is the cutoff associated with an alpha = .05?',
              answer(1.96, correct = T),
              answer(-1.96, correct = T),
              message = "In this distribution, all values greater than or equal to the absolute value of 1.96 would be considered significant at alpha = .05")
)
```
...
fluidRow(column(6,
  ...
  column(6, sliderInput('sdG1', 'Pooled SD', min=1, max = 5, step = 1, value = 2.5, ticks = F),
...
```{r q5, echo = FALSE}
quiz(caption = 'Test Your Understanding',
     question("If the difference between means and the sample size is held constant, what generally happens to the effect size when one group's SD is increased?",
...
Let's think back to our instructor, who adapted Assessment C for use in their course. The instructor would like to know the average Assessment score for all students who have ever taken and will ever take the course. Although that's not possible, they can estimate this value by giving their current students the assessment and inspecting the average score.
...
quiz(caption = 'Test Your Understanding',
     question("In the above example, which of the following is the sample?",
              ...
              answer("The mean score for all students who have ever taken and will ever take the course"),
      ...
      question("In the above example, which of the following is considered the population parameter?",
...
To demonstrate what the confidence interval tells us, we need to take multiple samples from the population and calculate a confidence interval from each. Given what a confidence interval is telling us, if we were to take 100 samples and compute a confidence interval with each, we would expect around 95 of those confidence intervals to include a value of 57.97. Use the button below to generate a new set of samples.


```{r, echo = F, fig.height=20}
actionButton("do", "Collect 20 New Samples")
plotOutput('ciPlot')
```
