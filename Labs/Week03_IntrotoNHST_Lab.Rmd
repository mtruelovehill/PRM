...
output$aucScorePlot <- renderPlot({
  loLim <- ifelse(input$score[1]==20, 20, input$score[1])
  hiLim <- ifelse(input$score[2]==96, 96, input$score[2])


  propVal <- round((pnorm(input$score[2], mean = 58, sd = 9.4)-pnorm(input$score[1], mean = 58, sd = 9.4))*100)

  ggplot(data.frame(x=c(20, 96)), aes(x)) +
...
Recall that you had 45 students who completed each of the four assessments.  the plot below reflects the probability distribution of performance on Assessment A. 

### Your Tasks
Use this plot to answer the following questions:

MONICA - need to make it accurately reflect the distribution. Because it's 'real' data, using the dnorm function is skewing it just a bit, so just make a density curve that reflects the actual data. It should still provide the same understanding of the area under the curve.


```{r, echo = F}
sliderInput('score', 'Assessment C Score:', min=20, max = 96, step = 1, value = c(38,78), ticks = F)
