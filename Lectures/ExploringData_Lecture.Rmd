...
+ ...with a lot of uncertainty
  ...
  + We can't rely on deterministic models or intuition
  + We must gather data and look for consistent patterns within the data that support (or refute) our ideas
...
+ **Today:** Visualisations and Desciptive Summaries (used in Quantitative Analysis)
...
.center[
```{r, echo = F}
knitr::include_graphics('images/Nominal.png')
```
]


+ Data with no numeric value

+ Each category may be represented numerically, but categories themselves have no numeric relationship

--

+ **Examples of questions that produce nominal data:** 
  + Do you own a pet?
...
.center[
```{r, echo = F}
knitr::include_graphics('images/Ordinal.png')
```
]

+ Data may or may not be numeric

+ Data have some kind of order

+ Equal/consistent intervals between variables cannot be assumed

--

+ **Examples of questions that produce ordinal data:** 
  + On a scale between 1 and 7, rate how engaging you found the lecture.
...
+ Variables have equal and consistent intervals between values

+ There is no true zero

--

+ **Examples of questions that produce interval data:** 
...
.center[
```{r, echo = F}
knitr::include_graphics('images/Ratio.png')
```
]
...
+ Zero is a meaningful value and reflects the absence of the variable.

--

+ **Examples of questions that produce ratio data:** 
  + What is your age?
...
+ You can gain different information from your data depending on how it is visualised. 
  ...
  + **Frequency distributions** allow the visualisation of how often each observation occurs or how a variable is distributed
  + **Boxplots** and **Scatterplots** allow the visualisation of relationships between two (or more) variables
...
+ Displays the number of observations within the data that fall within each category or range

  + **x-axis:** Variable being visualised
...
+ The type of frequency distribution you should use depends on the type of data being visualised
...
+ Should be used with _continuous/numeric_ data (i.e. interval and ratio data)
...
> **Test your understanding:** What type of data will this produce?
...
> What does an average of 3.2 actually mean, in the context of this scale?
...
+ The mean is not ideal for describing ordinal data, even when they are coded as numeric values

+ **Common Exception:** When ordinal values are summed to produce an approximately continuous variable, such as a composite score on a scale.
]
...
+ Could be used with ordinal, ratio, or interval data
...
+ In general, how far away are individual observations from the mean? 
...
+ In general, participants in our sample are `r round(sqrt(sum(sampDat$sqDist)/(nrow(sampDat)-1)),2)` years older or younger than the mean of `r round(mean(sampDat$Age),2)`.
