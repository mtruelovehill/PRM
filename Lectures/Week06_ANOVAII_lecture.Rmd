...
+ Calculating $SS_{model}$ - the variance in the DV explained by the IV
...
.pull-right[
When there are multiple IVs, $SS_{model}$ is calculated as the distance between the means of **each independent group** and the grand mean. 

With 2 IVs (one with 2 levels and one with 3), we have $2\times3 = 6$ possible groups, so $SS_{model}$ is computed from these:

```{r, echo = F, fig.height=4, warning = F}
set.seed(526)
facDat <- data.frame(IV = c(rep('Cats/Extrovert', 4), rep('Cats/Introvert', 4), rep('Dogs/Extrovert', 4), 
                            rep('Dogs/Introvert', 4), rep('None/Extrovert', 4), rep('None/Introvert', 4)),
                  DV = c(rnorm(4, mean = 15, sd = 3), rnorm(4, mean = 18, sd = 3), rnorm(4, mean = 12, sd = 3)),
                  ID = 1:12)
