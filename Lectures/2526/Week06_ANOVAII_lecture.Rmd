...
+ Factorial ANOVA tests for differences in the dependent variable across <b>multiple</b> independent variables
...
+ The researcher wants to test for 3 possible effects:
  ...
  + Is there an effect of pet type(dog/cat/none) on subjective well-being?
  
  + Is there an effect of personality(introvert/extrovert) on subjective well-being?
  
  + Does the effect of pet type on subjective well-being differ depending upon personality?
...
+ With a Factorial ANOVA, the only difference is that $SS_{model}$ is comprised of the variance from each independent variable and their interaction(s):
...
.pull-left[
+ As with One-Way ANOVAs, $SS_{model}$  is calculated as the distance between the means of each group and the grand mean. 
...
```{r, echo = F, warning = F, fig.height=5, fig.width = 7}
ssMod + theme(legend.position = 'none') +
  annotate(geom = 'text', label = 'SS Model (Variance in DV explained by IVs)', x = 9, y = 32, size = 5)
```
...
.pull-left[
We can compute $SS_{IV1}$ (Pet) and $SS_{IV2}$ (Personality)...
...
.pull-right[
and we can also compute $SS_{model}$
```{r, echo = F, warning = F, fig.height=4.5, fig.width = 7}
ssMod + theme(legend.position = 'none') +
  annotate(geom = 'text', label = 'SS Model (Variance in DV explained by IVs)', x = 9, y = 32, size = 5)
```
]
...
We can rearrange the equation and use these values to compute the interaction effect: 
$$SS_{model} - (SS_{IV1} + SS_{IV2}) = SS_{IV1\times IV2}$$
...
+ For this example, let's run an a priori power analysis to detect effect size.
